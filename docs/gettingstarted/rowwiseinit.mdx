...
Creating an example dataset rowwise to enable testing.  The `SAS`, `Python`, `R`, and `Julia` examples all return a dataset / dataframe.  The `bash` example creates a csv file that can be [imported](../io/readcsv) into any of these languages or simply manipulated with other shell commands.
